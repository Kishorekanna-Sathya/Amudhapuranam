...
export default function ForceGraph({
  ...
  useEffect(() => {
    ...
    // Wait one animation frame so the container has been painted and has real dimensions
    rafId = requestAnimationFrame(() => {
      ...
      // ── Simulation data ─────────────────────────────────
      // Seed nodes at canvas center with small random spread
      ...
      // ── Simulation — pre-settle synchronously ────────────
      sim = d3
        .forceSimulation(nodes)
        .force("link", d3.forceLink(links).id((d: d3.SimulationNodeDatum) => (d as Character).id).distance(180).strength(0.3))
        .force("charge", d3.forceManyBody().strength(-600))
        .force("center", d3.forceCenter(W / 2, H / 2).strength(1))
        .force("collision", d3.forceCollide(65))
        .stop();

      // Run simulation synchronously to produce a settled layout
      const tickCount = Math.ceil(Math.log(sim.alphaMin()) / Math.log(1 - sim.alphaDecay()));
      for (let i = 0; i < Math.min(tickCount, 300); i++) sim.tick();

      // ── Zoom-to-fit immediately (no waiting for async end) ──
      const applyZoomToFit = () => {
        ...
        const scale = Math.min(0.9, scaleX, scaleY);
...
// ── Links ─────────────────────────────────────────────
      ...
      // Draw the pre-settled positions immediately (no blank frame)
      tick();

      // Restart simulation for gentle settling animation
      sim
        .alpha(0.3)
        .on("tick", tick)
        .restart();
